...
namespace Mps
{
    public class Mp2_XfromControl : MonoBehaviour
    {
        public Toggle T, R, S;
        public Mp2_SliderWithEcho X, Y, Z;
        ...
        private void UISetObjectXform(ref Vector3 p, ref Quaternion q)
        {
            ...
            {
                //ASL will do the *= part to ensure proper rotation. If you do that yourself, you may get weird rotation values
                Quaternion tempQuaternion = q;
                mSelected.GetComponent<ASL.ASLObject>().SendAndSetClaim(() =>
                {
                    mSelected.GetComponent<ASL.ASLObject>().SendAndSetLocalRotation(tempQuaternion);
                });
            }
        }

        void OnRelease(GameObject _myGameObject)
        {
            //Update Xform to show release
            mSelected = null;
