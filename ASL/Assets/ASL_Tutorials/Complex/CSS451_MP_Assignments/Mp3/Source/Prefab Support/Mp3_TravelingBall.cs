...
namespace Mps
{
    public class Mp3_TravelingBall : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            bool castShadow = mWorld.ProcessBarrier(GetComponent<Mp3_TravelingBall>(), mShadow.transform);
            mShadow.GetComponent<Renderer>().enabled = castShadow;

            castShadow = mWorld.ProcessBigLine(GetComponent<Mp3_TravelingBall>(), mProjectedOnBig.transform);
        ...
        public void Initialize(Mp3_TheWorld w)
        {
            Mp3_LineSegment l = w.GetAimLine();
            mWorld = w;
            SetDir(l.GetLineDir());
