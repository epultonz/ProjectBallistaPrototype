...
namespace ASL
{
    ...
    public class LobbyManager : MonoBehaviour
    {
        ...
        private enum PanelSelection { MainMenu, ConnectionMenu, HostMenu, RoomView, LobbyScreen };
        ...
        private GameObject m_ConnectButton = null;
        ...
        /// <summary> The screen the player sees after they have joined a matc.h </summary>
        ...
        private GameObject m_ReadyButton = null;
        ...
        void Start()
        {
            ...
            {               
                ...
                {
                    Disconnect(m_ChosenMatch.shortCode);
                }
            ...
            {
                //Move the user to the room they just created
        ...
        private void HostRoom()
        {
            ...
            string roomName = m_RoomNameInputField.GetComponentsInChildren<Text>()[1].text;
            GSRequestData matchData = new GSRequestData();
            matchData.AddString("roomName", roomName);
                ...
                .Send((_nameResponse) =>
                {
                    ...
                    new MatchmakingRequest()
                        ...
                        .Send((_matchMakingResponse) =>
                        {
                            ...
                            else //Host was succesful - update lobby screen 
                            {
                                ...
                                m_MatchNameText.GetComponent<Text>().text = matchShortCode + ": " + roomName;
                    ...
                    .Send((_findMatchResponse) =>
                    {
                        ...
                        else
                        {
                            foreach (var match in _findMatchResponse.PendingMatches)
                            {
                                ...
                                if (match.MatchData.GetString("roomName")?.ToString() == null)
                                {
                                    continue;
                                }
                                ...
                                buttonText[1].text = match.MatchData.GetString("roomName");
        ...
        private void JoinMatch()
        {
            new JoinPendingMatchRequest()
                ...
                .Send((_JoinPendingMatchResponse) =>
                {
                    ...
                    {
                        //Disconnect from the matches we are "hosting" - remember we had to create matches to see matches
                        foreach (string shortCode in m_MatchShortCodes)
                        {
                            if (m_ChosenMatch.shortCode != shortCode) //Don't disconnect from the match we just connected to
                            {
                                Disconnect(shortCode);
                            }
                        }
                        DisplayPanel(PanelSelection.LobbyScreen);
                        GameSparksManager.Instance().SetMatchShortCode(m_ChosenMatch.shortCode);
        ...
        private void Disconnect(string _shortCode)
        {
            new LogEventRequest().SetEventKey("Disconnect").SetEventAttribute("MatchShortCode", _shortCode).Send((_disconnectResponse) => {});
        }
        ...
        private void OnMatchFound(GameSparks.Api.Messages.MatchFoundMessage _message)
        {
            ...
            new LogEventRequest()
                ...
                .Send((_nameResponse) =>
                {
                    ...
                    m_MatchNameText.GetComponent<Text>().text = _message.MatchShortCode + ": " + _message.MatchData.GetString("cleanRoomName");
                    Debug.Log("Room Name: " + _message.MatchData.GetString("cleanRoomName"));
                });
        ...
        /// </summary>
        /// <param name="_response">The callback variable from the GameSparks API.</param>
        private void OnAuthentication(AuthenticationResponse _response)
        {
            //Update user's information for display
            m_UserNameText.GetComponent<Text>().text = "Username: " + _response.DisplayName;
            m_ConnectionStatusText.GetComponent<Text>().text = "Connection Status: Connected";
        ...
        private void Connect()
        {
            ...
            {
                GSRequestData matchData = new GSRequestData();
                string cleanRoomName = Regex.Replace(m_RoomName, @"\s+", "");
                matchData.AddString("cleanRoomName", cleanRoomName);
                new MatchmakingRequest()
                .SetMatchShortCode("QS") //Quick Start short code
                .SetSkill(0)
                .SetMatchData(matchData)
                .SetMatchGroup(cleanRoomName)
                .Send((_matchMakingResponse) =>
                {
                    if (_matchMakingResponse.HasErrors)
                    {
        ...
        private void SetupHostDropDownMatchOptions()
        {
            ...
            new LogEventRequest()
                ...
                .Send((_nameResponse) =>
                {
                    ...
                    names.m_Names.Remove("Quick Start"); //Since Quick start uses auto match making, we don't want to chose it
        ...
        private void OnApplicationQuit()
        {
            ...
            if (m_MatchShortCodes != null)
            {
                foreach (string match in m_MatchShortCodes)
                {
                    new LogEventRequest().SetEventKey("Disconnect").SetEventAttribute("MatchShortCode", match).Send((_disconnectResponse) => { });
                }
            }
